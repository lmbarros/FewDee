...
void main()
{
   al_run_allegro(
   {
      ...
      // Initialize the resources we'll use. We are not using the
      // 'ResourceManager' here, so we'll have to release these resources
      // manually when the program ends.
      ...
      SpriteNode[] stars;
      SpriteNode[] smallClouds;
      SpriteNode[] largeClouds;
      ...
      // these helper functions take a single parameter, which is the position
      // of the object along the horizontal axis (when the program is already
      // running, all objects will be created slightly beyond the screen's right
      // corned, out of the user view; but when firstly populating the scene, we
      // want stars and clouds all over the screen).
      //
      // Each of the helper functions creates a new SpriteNode, adds the proper
      // bitmap to it, sets its position (including the 'z' coordinate, which
      // determines the drawing order and is what we are testing here), adds it
      // to the proper array of objects and adds it as a child of the scene
      // graph root.
      ...
      // random number as parameter to the helper functions (this is the
      ...
      // creation of objects of the corresponding type. (As you'll see below,
      // the actual intervals have a random factor, to avoid making things too
      // regular; the values defined here are just a rough specification of the
      // actual intervals used.)
      enum timeToCreateStar = 0.953;
      enum timeToCreateSmallCloud = 2.5;
      enum timeToCreateLargeCloud = 5.0;

      // And these are the variables used to control how much time we still have
      // to wait until creating a new object. When one of those "counters" gets
      // to zero, one instance of the corresponding object is created.
      ...
      EventManager.addHandler(
         ...
         delegate(in ref ALLEGRO_EVENT event)
         {
            // Decrement the "counters" used to determine if it is already time
            ...
            // are used together with a random distribution to generate the time
            // of the next object creation.
            if (starCountdown <= 0.0)
            {
               starCountdown =
                  uniform(timeToCreateStar / 2, timeToCreateStar * 2);
            ...
            {
               smallCloudCountdown =
                  uniform(timeToCreateSmallCloud / 2,
            ...
            {
               largeCloudCountdown =
                  uniform(timeToCreateLargeCloud / 2,
                          timeToCreateLargeCloud * 2);
               addLargeCloud(WIDTH + LARGE_CLOUD_SIZE);
            }

            // Update stars: move each one to the left (at the proper speed),
            // and, if they went beyond the left screen corner, remove them from
            // the scene graph ('makeOrphan()') and from the array of objects
