...
void main()
{
   al_run_allegro(
   {
      ...
      // Handle draw events. We simply clear the screen and print the same kind
      // of information as we did for tick events.
      EventManager.addHandler(
         FEWDEE_EVENT_DRAW,
         delegate(in ref ALLEGRO_EVENT event)
         {
      ...
      // Now, what everyone was expecting: the main game loops! (I'll not tell
      // all the details about them here; check the documentation!)
      //
      // Notice that, in addition to the loop versions shown here, FewDee
      // provides handy overloads for those using the 'StateManager'. You may
      // want to check those out, too!
      //
      // And there is also a simple 'run()', which is simply an alias to
      // 'runWithVariableDrawAndTickRates()'.

      // All main loops take as parameter a delegate which is used to detect
      // when to exit the loop. Here we define the function used for that
      // purpose. (For such a simple function, passing a lambda would be a good
      // choice; other examples do just that.)
      bool keepRunning()
      {
         return !exitPlease;
      }

      // Some loop implementations try to keep a constant tick rate. However,
      // when running in slower hardware, they may not be able to keep with the
      // desired pace. In these cases, the loop will keep running, but slower
      // than expected.
      //
      ...
      // long was the previous tick, in seconds (that is, the time elapsed from
      // the previous tick to the current one); it allows you to determine how
      // slower than the expected you are going. The second parameter,
      // 'totalTime' is the time, in seconds, elapsed since some arbitrary point
      // in time; it allows you to ignore too frequent calls to the handler.
      //
      ...
      version (FewDee_runWithConstantDrawAndTickRates)
      {
         ...
         // (ticks per second). The "running behind handler" will be called if
         // the computer running the code is not fast enough to maintain the
         // desired frame/tick rate.
      ...
      {
         // This loop generates "draw" and "tick" events as fast as possible,
         // but the intervals between draws/ticks will vary as the workload
         // varies.
         runWithVariableDrawAndTickRates(&keepRunning);
      }
      else version (FewDee_runWithFixedTickRateAndMaximumDrawRate)
      {
         // This loop generates "tick" events at a fixed rate ('desiredTPS'),
         // but draws as fast as possible. Just keep in mind that, in the
         // simplest usage, even though the frame rate is technically as high as
         // possible, the real frame rate will not be greater than the tick
         // rate, because the same frames will be rendered multiple times if the
         // game state is not updated between different "draw" events. State
         // prediction via interpolation may be a solution to this (given that
         // you know the tick rate, and the draw handler receives as parameter
         // the time elapsed since the last draw, you can estimate how much time
         // elapsed since the last time the game state was updated).
      ...
      else version (FewDee_customLoop)
      {
         ...
         // 'EventManager.triggerTickEvent()' and
         // 'EventManager.triggerDrawEvent()' to generate, respectively, tick
         // and draw events.
         //
