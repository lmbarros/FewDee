...
void main()
{
   ...
   // Here we create the first mapping between low-level events and high-level
   // commands. In this case, we are saying that when the space bar is pressed,
   // then a 'JUMP' command must be triggered.
   ...
   // Add some more mappings. Notice that we are adding alternative mappings for
   // the same commands. Specifically, pressing joystick buttons will cause
   // 'JUMP' and 'FIRE' high-level commands.
   abstractedInput.addMapping(joyButtonPress(0), TheCommands.JUMP);
   abstractedInput.addMapping(joyButtonPress(1), TheCommands.FIRE);

   // Up to this point, we just told the 'AbstractedInput' how to generate the
   // high-level commands we want to handle from the low-level events that
   // Allegro generates. We still have to tell 'AbstractedInput' what we want
   // done when high-level commands are triggered. That's what we'll do next.
