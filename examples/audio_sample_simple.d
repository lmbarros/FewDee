...
void main()
{
   ...
   // instantiated via the AudioManager.
   //
   // Here, we use the AudioManager.play() method to create an audio sample
   // instance that will start playing immediately. The AudioSampleInstance
   // returned by play() will be automatically destroyed by the AudioManager, so
   // you don't have to worry about managing its memory (at least not in the
   // simple usage we are showing here: check the docs for the full story).
   //
   // One final note: if we simply wanted to play the sound without accessing
   // any of the services provided by AudioSampleInstance, we wouldn't even need
   // to store the AudioSampleInstance returned by play(). In this case, we'll
   // need to access one of its properties, so we store it.
   AudioSampleInstance audioSampleInstance = AudioManager.play(audioSample);
