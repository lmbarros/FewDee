...
void main()
{
   ...
   // 'TickBasedUpdater' registers itself with the 'EventManager' so that it can
   // run the updater functions as it gets "tick" events
   // (FEWDEE_EVENT_TICK). You can think of 'TickBasedUpdater' as an automated
   // 'Updater'.
   ...
   // Load a font. We are not using the 'ResourceManager', so we'll have to free
   // it manually latter.
   ...
   // Start an updater when the mouse is clicked. Notice that many instances of
   // the updater function will run simultaneously if the user clicks several
   // times in sequence.
      ...
      delegate(in ref ALLEGRO_EVENT event)
      {
         ...
         immutable id = updater.add(
            delegate(double deltaTime)
            {
               ...
               // When an updater function returns 'true', it means "please keep
               // calling me"; when it returns 'false', it is actually saying
               // "I'm done, don't call me anymore."
               return totalTime < 1.5;
            });

         // Print the ID of the updater function added. This value can be passed
         // to the updater's 'remove()' method, if you want to remove an updater
         // before it considers itself done.
   ...
   // Just draw some instructions. All interesting output goes to the console,
