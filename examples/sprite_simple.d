...
void main()
{
   ...
   // Initialize the only resource we'll use. This is the sprite sheet we'll use
   // with our sprites. We are not using the 'ResourceManager' here, so we'll
   // have to release this bitmap manually when the program ends.
   auto bmpStickMan = new Bitmap("data/stick_man.png");

   // First thing to do is to create a 'SpriteTemplate'. A 'SpriteTemplate'
   // contains a collection of images and a collection of animations that can be
   // shared among several sprite instances. The constructor we are using here
   // sets the size of the images used by this sprite (all images must be of the
   // same size).
   scope sptStickMan = new SpriteTemplate(64, 64);

   // Now, we add the images. Notice that all the images are actually
   // referencing the same bitmap (our sprite sheet). The two numeric parameters
   // indicate the top left corner of the image within the bitmap.
   ...
   // updater function that will update the sprite's current image as the time
   // passes. In addition to the target sprite and the animation name, we pass
   // the relative speed at which we want to play the animation (we use 1.0,
   // since we want to play at the nominal speed) and a flag indicating that we
   // want to play in loop (true).
      ...
      delegate(in ref ALLEGRO_EVENT event)
      {
         switch (event.keyboard.keycode)
         {
            ...
            default:
            {
               ...
               // that's the default. In the case of 'sprStickMan3', which will
               // play the jump animation at its nominal speed, we don't even
               // have to pass 1.0 for the speed -- again, that's the default.
               //
   ...
   // Finally, set the drawing function. We simply clear the background to white
   // and draw each of the stick men, side-by-side.
   EventManager.addHandler(
      FEWDEE_EVENT_DRAW,
      delegate(in ref ALLEGRO_EVENT event)
      {
